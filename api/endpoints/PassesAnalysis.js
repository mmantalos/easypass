...
function getPassesAnalysis(req,res){
    ...
    con.connect(function(err) {
    	...
    	let myquery = `SELECT p.pass_id AS PassID, s.station_id AS StationID, p.timestamp AS TimeStamp, v.vehicle_id AS VehicleID, p.charge AS Charge FROM vehicles AS v, stations AS s, passes AS p WHERE v.vehicle_id = p.vehicle_ref AND s.station_id = p.station_ref AND v.tag_provider = ${req.params["op2_ID"]} AND s.station_provider = ${req.params["op1_ID"]} AND CAST(p.timestamp AS date) BETWEEN ${req.params["date_from"]} AND ${req.params["date_to"]};`;
